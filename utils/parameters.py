...
class Account:
    def __init__(
        ...
        self.compound: MonthlyCompoundType = compound_type
...
class Person:
    ...
    def add_account(self, account: Account, account_name: Optional[str] = None) -> None:
        ...
        if account.type == AccountType.TRADITIONAL or account.type == AccountType.HSA:
    ...
    def get_fica_taxable_income(self) -> float:
        ...
        for _, account in self.accounts.items():
            if account.type == AccountType.HSA:
        ...
        return self.pre_tax_income - total_hsa_contribution
