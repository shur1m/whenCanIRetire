...
def _calculate_pre_tax_income(post_tax_income: int):
    ...
    while abs(right - left) > 0.01:
        ...
        income_tax = calculate_annual_income_tax(Person(pre_tax_income=pre_tax_income))
        if pre_tax_income - income_tax > post_tax_income:
...
def _simulate_accumulation(
    ...
    if not account.owner:
        raise Exception("simulated accounts must have an owner")
    ...
    for year in range(account.owner.retirement_age - account.owner.current_age):
        ...
        for month in range(12):
            ...
            if account.compound_frequency == Frequency.MONTHLY:
                if account.compound == MonthlyCompoundType.DIVIDE:
                    current_savings *= 1 + account.annual_investment_return / 12
                elif account.compound == MonthlyCompoundType.ROOT:
...
def _simulate_retirement(
    ...
    # binary search pre tax expense given post tax expense for Generic and
    # Traditional account withdrawals, Roth/HSA withdrawals are not adjusted
    if account.type == AccountType.GENERIC or account.type == AccountType.TRADITIONAL:
    ...
    while (
        ...
        years_since_today = account.owner.retirement_age - account.owner.current_age
        months_since_today = years_since_today * 12 + retirement_months
