...
def init_ui():
    ...
    async def on_year_change(new_year: str):
    ...
    header_save_btn = render_header(
        state=app_state,
        on_year_change=lambda val: asyncio.create_task(on_year_change(val)),
        on_save=lambda: asyncio.create_task(save_and_calculate()),
    )
        ...
        def render_config_form():
            ...
            with form_container:
                ...
                drawer_save_btn = ui.button(
                    "Save & Calculate",
                    on_click=lambda: asyncio.create_task(save_and_calculate()),
    ...
    ui.timer(0.01, lambda: asyncio.create_task(update_charts()), once=True)
